...
void Matrix4::multiply(Vector3 &vector) {
...
Matrix4 Matrix4::operator*(Vector3 &other) {
    float mBuf[4][4];
    float* vec = other.get4();
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            mBuf[i][j] = this->matrix[i][0] * vec[i] + this->matrix[i][1] * vec[i] + this->matrix[i][2] * vec[i] + this->matrix[i][3] * vec[i];
        }
    }
